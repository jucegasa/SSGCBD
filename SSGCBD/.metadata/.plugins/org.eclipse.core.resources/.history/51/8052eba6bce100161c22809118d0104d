...
public class DataformDiagramGenerator {
	...
	public Node createComboView(int x, int y, int width, int height) {
		...
		DescriptionStyle descriotionSt = createDescriptionStyle();
		FillStyle fillSt = createFillStyle();
		FontStyle fontSt = createFontStyle();
		...
		node.insertChild(bcAtributes);
		node.insertChild(bcMethods);
		node.getStyles().add(descriotionSt);
		node.getStyles().add(fillSt);
		node.getStyles().add(fontSt);
